...
export function ContactButtons() {
  ...
  return (
    <div className="fixed bottom-24 right-5 z-20 flex flex-col items-center gap-3 sm:bottom-5">
      <a href={`tel:${number}`} aria-label="Call Bite Corner" className="flex h-14 w-14 items-center justify-center rounded-full bg-red-500 shadow-lifted transition hover:scale-105 active:scale-95">
        <Phone className="h-6 w-6 fill-white text-white" aria-hidden="true" />
      </a>
      <a
        href={whatsappHref}
        target="_blank"
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-lifted transition hover:scale-105 active:scale-95"
      >
        <WhatsAppIcon />
      </a>
    </div>
